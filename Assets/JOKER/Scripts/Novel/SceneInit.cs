...
public class SceneInit : MonoBehaviour {
	...
	public void clearCurrentMessage(){
		...
		if (obj == null) {
			...
			c = obj.GetComponent<GUIText> ().guiText.color;
			name = obj.guiText.text;
		}
		//バックログ用
	...
	private IEnumerator showMessage(string message){
		...
		for (int i = 0; i < message.Length; i++) {
			...
			this.gameView.messageArea.guiText.text = this.currentMessage;
		...
		if (Input.GetMouseButtonDown (0)) {
			...
			if (aCollider2d) {
				...
				if (StatusManager.enableEventClick == true && name == "_sp_button_close") {
					...
					this.gameView.hideMessageWithoutNextOrder (0.2f);
					return;
				}
